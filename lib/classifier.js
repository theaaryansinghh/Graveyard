const CLASSIFIER = (() => {
  ...
  //signals that email is prob job related
  const JOB_SIGNALS = {
    subject: [
    ...
    //domains from ats systems recruiters use
    senderDomains: [
  ...
  //helper to check if any regex matches
  ...
  //checks if email is even job related
  function isJobEmail(email) {
    ...
    //most ats mails come from these domains
  ...
  function extractCompany(email) {
    ...
    if (fromName) {
      ...
      //cases like "john from acme corp"
      const atMatch = fromName.match(/(?:from|at|@)\s+(.+)/i);

      if (atMatch) return atMatch[1].trim();

      //if single word maybe company not person
    ...
    if (emailMatch) {
      ...
      //cleaning domain to look like company name
      return domain
